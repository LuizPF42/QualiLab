...
- **graficos** — `ChartsPanel`: **explorador** com layout `.split` de dois panes. **Esquerda** (`flex:0 0 270px`, mesmo tamanho da Visualização): painel de **filtro por categorias** — um `<select>` por categoria de opção (`closedCats` = `CAT_HAS_OPTIONS`), montado igual ao `VizNav`; o estado `catFilter` deriva `matchingDocIds` (mesma lógica do `VizNav`/`VizExcerpts`) e **restringe TODOS os gráficos aos documentos que passam** (via `fdocs` e `cod` já filtrado por `matchingDocIds`; `indiv` também). Mostra "X de Y documento(s) no filtro" e "limpar filtros". **Direita**: um gráfico por vez via `.seg` de abas (`tab`: **Frequência · Co-ocorrência · Cobertura · Código × atributo · Tempo** quando há categoria de data · **Codificadores** só em coletivo, com sub-modo produção/concordância). Tudo desenhado **em SVG na mão** e **responsivo à largura** (helpers `svgHBars`/`svgHeat` recebem `w`/`W` medidos por `ResizeObserver` no `chartRef` — efeito depende de `empty` p/ re-anexar quando o card surge; `chTrunc`/`mergedLen`), **sem libs** — substituiu o `barRows`/tabela antigos e habilita **exportar SVG/PNG** (`exportChartSVG`/`exportChartPNG`, via `svgCloneForExport`+`inlineSvgStyles` que resolvem as `var(--cor)` computadas e desenham fundo branco; PNG rasteriza num `<canvas>` 2×). **Cobertura** = % do corpus coberto por código, mesclando sobreposições (`mergedLen`), métrica de peso real (não só contagem). Recorte "Ver:" compartilhado (`codingsForView`) e cores dos códigos (`codeColor`) preservados. (O painel de trechos por clique foi removido por ora — a ligação gráfico→trechos será reavaliada por tipo.)
