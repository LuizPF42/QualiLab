...
> 📥 **Reenviar o asset do release a cada regeneração.** O link "Baixe a ferramenta" do `README.md` aponta para um **asset de release** (tag `alpha`: `https://github.com/LuizPF42/QualiLab/releases/download/alpha/index.html`), que é um arquivo **congelado** — ele NÃO acompanha o `docs/index.html` automaticamente. O link "Acesse a ferramenta" (GitHub Pages) serve o `docs/index.html` atual. Então, **sempre que regenerar e commitar o `docs/index.html`, reenvie o mesmo arquivo como asset do release `alpha`** (Releases → editar `alpha` → apagar o `index.html` antigo → subir o novo), senão quem baixa pega uma versão mais velha do que quem acessa online. Conferir que bateu: `sha256sum docs/index.html` deve igualar o `digest` do asset (Releases → o `sha256:` do `index.html`). Já divergiram uma vez por esse passo ter sido pulado.
