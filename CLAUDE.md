...
> 🔗 **Este `docs/index.html` é GERADO — NÃO edite aqui.** A fonte única é o `index.html` do fork **`../QualiLab-ai-lab`** (superset = estável + camada de IA). Este arquivo é regenerado por `QualiLab-ai-lab/scripts/gen-estavel.sh`: desde jul/2026 é o **mesmo** `index.html` do ai-lab, apenas com as **credenciais Supabase deste projeto público** trocadas — o script **não mexe mais em `AI_ENABLED`**. **A IA está LIGADA aqui** (`AI_ENABLED = true`), em modo **BYOK** — cada pesquisador traz a própria chave; `AI_SHARED_KEY = false` = a UI não oferece nenhuma chave de servidor (nem menciona instituição). Isso exige (e já tem) a Edge Function `ai-ask` deployada no Supabase público + as tabelas de IA (`ia_results`/`ia_memory`, escopos `ai_*`/`coding` em `memos`). **Para mudar qualquer coisa, edite no ai-lab e rode o script** — edições feitas aqui serão perdidas na próxima regeneração. Censura (`is_redaction`) é feature geral de codificação (não gateada por flag).

> 📥 **Asset do release x `docs/index.html` — agora sincronizado por CI.** O link "Baixe a ferramenta" do `README.md` aponta para um **asset de release** (tag `alpha`: `https://github.com/LuizPF42/QualiLab/releases/download/alpha/index.html`), que é um arquivo **congelado**; o link "Acesse a ferramenta" (GitHub Pages) serve o `docs/index.html` atual. Os dois já divergiram uma vez por o asset não ter sido reenviado. Agora o workflow **`.github/workflows/sync-release-asset.yml`** reenvia o `docs/index.html` como asset do release `alpha` (`gh release upload alpha ... --clobber`) **a cada push no `main` que toque o `docs/index.html`** — então, no fluxo normal (regenerar → commitar → push), o download fica em sincronia sozinho. Reenvio manual só é necessário como fallback: Actions → "Sincronizar asset do release" → *Run workflow* (`workflow_dispatch`), ou Releases → editar `alpha` → trocar o `index.html`. Conferir que bateu: `sha256sum docs/index.html` deve igualar o `digest` do asset (Releases → o `sha256:` do `index.html`).
...
**`store.onSaveError(cb)`** (`LocalStore`/`createFileStore`, mesmo padrão de `onPendingChange`): `localStorage.setItem`/`fileHandle.createWritable()` podem falhar (quota — geralmente 5-10MB — esgotada, storage desabilitado, permissão de arquivo revogada, disco removido) e isso **sempre passou em silêncio** (`catch(e){}` vazio) — o usuário continuava trabalhando sem saber que nada mais estava sendo persistido, só descobria ao reabrir e ver os dados faltando. Agora `save()` chama `_onSaveError(e)` no catch e `_onSaveError(null)` em todo save bem-sucedido (limpa o aviso quando o problema é resolvido); o `App` assina isso (junto com `onPendingChange`, no mesmo `useEffect`) em `saveError`, e mostra um banner vermelho persistente (sem botão de fechar — só desaparece quando um save volta a funcionar) com atalho pra baixar `.qualilab` na hora. **O mesmo callback alimenta o "✓ salvo HH:MM"** (jul/2026): `_onSaveError(null)` também seta `lastSaved` no App, exibido como `.meta` discreto no cabeçalho (só local/arquivo — quem vem do Word não confia em autosave silencioso; na nuvem cada operação já é uma chamada de rede com a pílula offline cobrindo a falha).

**`store.onUsage(cb)`** (só `LocalStore`, jul/2026): indicador **proativo** de quota — `save()`/`endBatch()` já têm o `json.length` em mãos e notificam o App (`localUsage`) com o tamanho em chars do JSON salvo (também capturado do `raw` na carga inicial; `cb(lastJsonLen)` imediato na assinatura). `LOCAL_QUOTA_CHARS = 5e6` (const module-level antes de `LocalStore`) é a régua (~5M chars ≈ quota típica). UI: % de uso no tooltip da pílula do projeto (quando >0) + pílula âmbar "armazenamento X% cheio" no cabeçalho quando ≥75%, com instrução de baixar `.qualilab` e migrar. Projeto local recém-criado não grava nada até a 1ª mudança real (`createProject` do LocalStore não salva) — o tooltip omite o % até lá, correto.
...
onSaveError(cb)             // Local/FileStore (avisa falha silenciosa de gravação; null = save ok → "✓ salvo")
onUsage(cb)                 // só LocalStore (chars do JSON salvo — indicador proativo de quota)
enableFolderBackup() · disableFolderBackup() · onBackupStatus(cb)   // só LocalStore
realtime(pid,onChange)      // só SupabaseStore
```

### App
Um componente Preact grande com todo o estado. Fases: `boot | welcome | auth | gate | work` (`welcome` = tela de entrada arquivo/nuvem/rascunho — ver "Modelo de armazenamento e tela de entrada"). A "tela" de trabalho é decidida por `mainView`: `codificar | reconciliar | visualizar | graficos | memos | esquema | relatorio`.
...
10. Componentes: `Auth`, `Welcome` (tela de entrada), `ServerConnect` (form de conexão ao Supabase pessoal, reusado no `Welcome` e no `Auth`), `Gate`, `AccountModal`, `ProjectModal`, `CategoriesPanel`, `CategoryEditor`/`DraggableCategoryList`, `CategoryValue`, `Reconcile`, `SchemaCodesView`/`SchemaTreeNode`/`SchemaBulkPanel` (aba Esquema → Códigos), `VizNav`/`VizCodeNode`/`VizExcerpts` (Visualização), `ChartsPanel` + helpers SVG (`svgHBars`/`svgHeat`/`chTrunc`/`mergedLen`/`exportChartSVG`/`exportChartPNG`), `ReportView`+`REPORT_GROUPS` (Relatório), `CodesPanel`/`TreeNode`/`RenameCode`, `MemoNav`/`MemoEditor` (Memos), `DebInput`, `DateInput`/`parseDate`, `AddPaste`.
...
- **CONFIG**: `SUPABASE_URL` e `SUPABASE_ANON_KEY` estão fixos no `index.html` apontando para o projeto do autor. A anon key é **pública por design** (protegida por RLS). Para um deploy novo, troque por outras credenciais (ou deixe em branco para forçar modo local).
- **Supabase PESSOAL (corrigido jul/2026 — a precedência era invertida):** credenciais salvas pelo card "Conexão (Supabase)" do `ProjectModal` (`localStorage` `qualilab:url`/`qualilab:key`) agora **VENCEM as embutidas** — antes o `||` curto-circuitava nas embutidas e o card era **inócuo/enganoso** no app publicado (o pesquisador digitava o Supabase dele e o app recarregava conectado no oficial, sem avisar). Flags module-level junto da CONFIG: **`CUSTOM_SUPABASE`** (true quando as creds efetivas vieram do localStorage e diferem das embutidas → pílula "nuvem pessoal" violeta + botão "Voltar ao servidor padrão"; apontar na mão pro próprio servidor padrão NÃO conta como pessoal) e **`FORCE_LOCAL`** (`qualilab:forcelocal='1'` → `wantCloud=false`, boot direto em modo local — é o que faz "Desconectar (modo local)" funcionar de verdade: com creds embutidas, só remover url/key não desconectava nada). Handlers no call site do `ProjectModal`: `onSaveCloud` (grava creds + limpa forcelocal), `onClearCloud` (seta forcelocal, MANTÉM as creds pessoais pra reconectar fácil), `onResetCloud` (descarta creds pessoais + forcelocal), `onReconnectCloud` (só remove forcelocal; botão "Reconectar à nuvem" aparece no card em modo local quando há creds efetivas). **Lembrete deste repo derivado:** tudo isso vive no `index.html` gerado — mudanças, só via ai-lab + `gen-estavel.sh`.
...
- **Cores** (variáveis em `:root`): a família `--teal*` guarda o **azul-marinho FGV** (nome "teal" é legado): `--teal #00427A`, `--teal-deep #002B5C`, `--teal-soft #E2ECF5`, e `--sky #1B9DD9` (azul-claro FGV). `--danger #B23A3A` para ações destrutivas. `--ok`/`--ok-soft` (verde) e `--violet`/`--violet-soft` (jul/2026, com variantes dark) existem pros **estados da pílula do projeto**: `.pill.mode-file` (verde, arquivo = dados no disco), `.pill.mode-cloud` (azul FGV, nuvem oficial), `.pill.mode-custom` (violeta, nuvem pessoal = `CUSTOM_SUPABASE`), `.pill.mode-offline` (âmbar, nuvem sem conexão — também usada na pílula de aviso de quota do modo rascunho); rascunho (localStorage) = neutra, sem classe. `.nudge` (âmbar-soft, faixa abaixo do header no modo rascunho) e `.btn.violet` (Conectar ao meu Supabase) são as classes novas do reenquadramento. **Ícones = glyphs de texto monocromáticos, NÃO emojis** (jul/2026, flat design): ao adicionar UI, use glyph de texto ou SVG inline de contorno (como os `.wicon`), nunca emoji. O rótulo continua dizendo o modo (cor é reforço, não canal único — a11y) e o `title` explica onde os dados vivem; o estado é derivado numa IIFE no render da pílula (`cs`/`csLabel`/`csTitle`). Cores dos códigos vêm de `HUES`/`codeColor` (matiz = família, luminosidade = profundidade) — **não** mexa nelas para branding.
...
- **graficos** — `ChartsPanel`: **explorador** com layout `.split` de dois panes. **Esquerda** (`flex:0 0 270px`, mesmo tamanho da Visualização): (a) **filtro por categorias** — um `<select>` por categoria de opção (`closedCats` = `CAT_HAS_OPTIONS`), montado igual ao `VizNav`; `catFilter` deriva `matchingDocIds` (mesma lógica do `VizNav`/`VizExcerpts`) e **restringe TODOS os gráficos aos documentos que passam** (via `fdocs` e `cod` já filtrado; `indiv` também), com "X de Y documento(s)" e "limpar filtros"; (b) **"Ignorar censura"** (`hideRedaction`, **ligado por padrão**) que tira de `cod`/`indiv` as codings de código `is_redaction`; (c) **só na aba Nuvem**, uma **árvore de códigos com multi-seleção por checkbox** (componente próprio `CloudCodeNode` — espelha o `VizCodeNode` em profundidade/visual, mas com checkbox e **sem** arrastar/excluir do `SchemaTreeNode`; `cloudCodes` é um `Set`; `codeCounts` agregados) que filtra a nuvem pela seleção (`cloudCodeSet` = seleção literal; vazio = todos). **`toggleFamily`/`familyOf`: marcar um código marca toda a subárvore (descendentes) e desmarcar tira a subárvore** — vale tanto p/ a Nuvem quanto p/ os eixos da Co-ocorrência; depois dá pra desmarcar filhos individualmente; (d) **só na aba Co-ocorrência**, **dois** seletores `CloudCodeNode` escolhem os **eixos** da matriz — `coocX` = colunas (X), `coocY` = linhas (Y); cada eixo vazio cai nos **12 mais frequentes**. A `cooc` conta, por célula `m[linhaY][colunaX]`, os pares de trechos sobrepostos (laço ordenado, sem dobra; diagonal do mesmo código fica vazia). **Direita**: um gráfico por vez via `.seg` de abas (`tab`: **Frequência · Nuvem · Co-ocorrência · Cobertura · Código × atributo · Tempo** quando há categoria de data · **Codificadores** só em coletivo, com sub-modo produção/concordância). Tudo **em SVG na mão, sem libs**, e **responsivo**: `bodyRef` (o `pane-body`, viewport de tamanho estável) mede largura **e** altura da área visível por `ResizeObserver` (`cw`/`ch`) com rAF+limiar e `scrollbar-gutter:stable` contra o loop "undelivered notifications". Helpers `svgHBars`/`svgHeat` (recebem `w`/`W`, `chTrunc`/`mergedLen`) e **`svgWordCloud`** (tokens dos trechos codificados via `tokenizeWords`+`STOPWORDS_PT`, **sem lematização**; cor herdada do **código predominante**; layout em espiral de Arquimedes com colisão por retângulo; renderiza com `preserveAspectRatio="meet"` num contêiner de altura = `ch` → **cabe na tela sem rolagem**; a espiral nasce na proporção da área pra sobrar pouca margem). **Exportar SVG/PNG** por gráfico (`exportChartSVG`/`exportChartPNG` via `svgCloneForExport`+`inlineSvgStyles`; PNG rasteriza num `<canvas>` 2×). **Cobertura** = % do corpus coberto por código (`mergedLen`, mescla sobreposições). Recorte "Ver:" (`codingsForView`) e cores (`codeColor`) preservados. **Gráfico → trechos (jul/2026):** clicar numa **barra** (Frequência, Cobertura, Concordância) ou **célula** (Co-ocorrência, Código × atributo) abre a **Visualização** já naquele código — o App passa `onOpenCode(codeId, opts)` ao `ChartsPanel`, que seta `vizSelCode` + `vizCross` (célula de co-ocorrência → par nos `crossCodes` do `VizExcerpts`) + `vizCatFilter` e troca `mainView`. **O filtro de categorias do gráfico VIAJA JUNTO** (`opts.filter=catFilter`; mesmo shape `{catId:valor}` do `vizCatFilter`) — senão a contagem da barra não bateria com os trechos exibidos; célula de Código × atributo ainda soma `{[catId]: valor da coluna}`. O "Ver:" já era estado compartilhado (`viewCoder`), consistente de graça. Nuvem (palavras), Tempo (anos) e Produção (pessoas) ficam sem clique de propósito — as barras não são códigos (Concordância é por código, então tem). `svgHBars`/`svgHeat` **já aceitavam `onClick`** (resíduo do painel antigo) — só foi fiação; os hints das abas clicáveis ganham "Clique … para ver os trechos".
...
A **pílula do projeto** no cabeçalho abre o `ProjectModal` (hub: convite/código de acesso, tipo, membros, renomear/limpar/excluir, conexão). O **nome do usuário** abre `AccountModal` (nome, senha, gestão de todos os projetos, **e "sair da conta"** — chama o mesmo fluxo de logout do botão "sair" do cabeçalho: `store.signOut()` + limpa projeto/usuário ativo e volta pra tela de auth). O menu **"exportar ▾"** no cabeçalho inclui o item **".qualilab (projeto completo, nativo)"**, que chama `saveQualilab()` (download do projeto inteiro em qualquer modo); o mesmo `saveQualilab()` é oferecido pelo atalho **"baixar .qualilab"** da faixa de `saveError`. (Não há mais um botão "salvar .qualilab" solto no cabeçalho.)

### Modelo de armazenamento e tela de entrada (`welcome`)

Reforma jul/2026: o `localStorage` foi reenquadrado como **"rascunho"** (efêmero, "só testar"); arquivo e nuvem são as opções de trabalho de verdade. Peças:

- **`Welcome`** (fase `welcome`, renderizada antes de `openFileProject`/`enterLocalOffline` no `App` pra tê-los em escopo): tela de entrada com **logo em lockup** (SVG inline do favicon — quadrado `#00427A` + "Q" branca — colado no wordmark "ualiLab" em `var(--teal)`; `aria-label="QualiLab"`), tagline e **três cards** (`.wcard`): "Novo em arquivo" e "Entrar na nuvem" **lado a lado** (pares), "Só testar (rascunho)" abaixo. Ícones SVG inline de contorno (`.wicon`, cor por modo), "Abrir arquivo existente" e rodapé (GitHub · MIT · autor). Degrada sem `supportsFileAPI` (Firefox/Safari: só nuvem+rascunho).
- **Boot reescrito**: `reopenLastFile()` roda **primeiro** (arquivo salvo = escolha deliberada → reabre em `mode='file'`); senão, `wantCloud` sem sessão → **`setPhase('welcome')`**; `!wantCloud` sem arquivo → rascunho automático. Rotas do welcome: arquivo → `openFileProject(true/false)`; nuvem → `setPhase('auth')`; rascunho → `enterLocalOffline()`.
- **Rascunho reframe**: pílula/subtítulo/tooltip dizem "rascunho"; `.nudge` (faixa âmbar-soft abaixo do header, `store?.mode==='local'`) com atalhos "Salvar como arquivo"/"Conectar à nuvem" + ✕ (`nudgeHidden`, colapsável por sessão).
- **Login limpo** (`Auth`): **removidos** "Continuar como visitante" e "Usar offline" (o rascunho do welcome cobre); ganhou **`onBack`** (← Voltar → welcome) e "Entrar" centralizado. `onAnon`/`onLocal` saíram das props.
- **Supabase pessoal pré-login** (`ServerConnect`, reusado em `Welcome`+`Auth`): botão violeta "Conectar ao meu Supabase" → form URL+anon key → `connectCustomCloud(url,key)` (grava `qualilab:url`/`key` + reload). Se já em `CUSTOM_SUPABASE`, mostra o estado + `resetCustomCloud` (voltar ao padrão). **Gotcha (TDZ):** `connectCustomCloud`/`resetCustomCloud` têm de ser definidos **antes** dos `return` de fase no `App` (o `Auth` os referencia).
- **Logout** (`AccountModal`, `onLogout` só em modo nuvem): os três caminhos de logout (conta, "sair" do cabeçalho, pré-work) vão pra tela de entrada (`welcome`).
...
Dois cards irmãos no `ProjectModal` trocam de armazenamento sem exportar/importar na mão: **"Salvar como arquivo (.qualilab)"** (`canSaveAsFile = supportsFileAPI && store?.mode==='local'`, só rascunho/Chromium) — handler `saveAsFile`: `buildQualilabDb()` → `showSaveFilePicker` → `createFileStore(fh,true)` → `mergeQualilabDb(...)` → `setStore(st)` + **`setMode('file')`** + `openProject(p)` (migra o rascunho pro arquivo num clique, pílula verde; desliga o backup-em-pasta); e **"Sair deste arquivo"** (`canLeaveFile = store?.mode==='file'`) — handler `leaveFile`: `IDB.deleteHandle('qualilab:file')` + reload, o escape do reabrir-automático (senão o boot reabriria o arquivo pra sempre). Entrar no modo arquivo seta `mode='file'` (em `openFileProject`/`saveAsFile`/boot-reopen), corrigindo o resíduo `'cloud'`/`'local'` — auditado: todos os checks `mode==='cloud'` tratam `'file'` como não-nuvem.
...
**Comentários viram memos.** `importQDPX` mapeia `<Description>` (comentário) de **documento** (`PDFSource`/`TextSource`) → memo de documento e de **código** (`Code`) → memo de código, via `store.setMemo(pid, scope, targetId, ...)` (um por alvo), remapeando os ids como o resto. Usa `:scope > Description` pra não pegar a Description de seleções (`PDFSelection`) nem de `Code` aninhado. As `<Notes>`/`<Note>` livres do projeto (cada uma com `name` + texto em `plainTextPath`) são **reunidas num único memo de PROJETO** (`scope='project'`, `target_id=null`), com títulos `## nome` e separador — anexando (não sobrescrevendo) um memo de projeto já existente. Só os comentários por **trecho** (Description de `PDFSelection`) ficam de fora — o modelo de memo do QualiLab não ancora num trecho. O `doImportQDPX` recarrega `setMemos(...)` após importar e conta no resumo. (No HU China: 36 memos de documento + 27 de código.)
...
```
Teste com `examples/QualiLab_synthetic_realistic_legal_ai_3.qualilab` (corpus fictício de demonstração: 9 docs, 22 códigos com 3 de censura, 288 trechos codificados, gabarito completo, co-ocorrência densa, 3 codificadores; importe por "Só testar (rascunho)" na tela de entrada → importar ▾ → .qualilab). Ele é gerado por `examples/gen_example_3.py` (edite o script e rode `py examples/gen_example_3.py` — nunca edite o `.qualilab` à mão; os offsets são computados). Não há suíte de testes automatizada — verificação é manual no navegador.
...
- `LocalStore` ainda usa `localStorage` (limite 5-10MB). Candidato a migrar pra IndexedDB — funciona em Safari igual, sem o teto baixo. O banner de `onSaveError` cobre o aviso reativo (avisa quando o save falha) e o indicador proativo de quota (`onUsage`, jul/2026) avisa **antes** — mas a migração pra IndexedDB removeria o teto de vez.
