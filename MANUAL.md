...
- **Limites de tamanho.** Há um teto por envio: cerca de **8.000 caracteres por documento** e **40.000 no total**. Seleções maiores são **cortadas para caber** (a lista de códigos fica sempre inteira; a amostra de trechos é reduzida primeiro), então, para uma análise completa, selecione menos material de cada vez.
...
**A Tarefa (o que você quer que a IA faça).** O campo da tarefa é **texto livre**: escreva o pedido com as suas palavras. Para não partir do zero, o botão **biblioteca de prompts ▾** abre uma lista de **sugestões prontas** (que se *inserem* no campo como ponto de partida editável) e dos **prompts que você salvou**; o botão **✦ salvar** guarda o texto atual como um prompt reutilizável (ele passa a aparecer aqui e na aba Memos → "Prompts salvos"). As sugestões variam conforme o escopo:
...
- **Postura** (Papel e princípios): *Padrão*, *Indutivo*, *Dedutivo*, *Abdutivo* ou *Personalizado* (abre um campo para você descrever a postura). É a lente metodológica da análise.
...
- **Prévia e proveniência**: abaixo dos controles, o prompt aparece **seção por seção**, com o **modelo** ativo, as contagens e a **estimativa de tokens** (com aviso se o material for truncado), e um botão **copiar prompt**.
