...
3. [A interface](#3-a-interface): cabeçalho, barra de status, paleta de comandos, painéis, teclado e acessibilidade
...
17. [Codificar e Analisar com IA](#17-codificar-e-analisar-com-ia): Auto-codificação, Analisar e Explorar com IA: opcional (BYOK), opt-in e transparente
...
1. Abra **[luizpf42.github.io/QualiLab](https://luizpf42.github.io/QualiLab)** e clique em **Só testar (rascunho)**. O QualiLab pergunta se o projeto terá **recursos de IA**; para começar, **Continuar sem IA** basta (dá para ativar depois). O projeto abre na hora, só neste navegador.
2. Na aba **Codificação**, clique em **colar texto…** e cole um trecho: uma fala de entrevista, um parágrafo de decisão, uma notícia (ou use **＋ enviar ▾** para um `.txt`, `.pdf` ou `.docx`, ou **experimentar com um corpus de exemplo** para ver grifos e gráficos prontos).
...
1. **Declare o objetivo** da pesquisa num **memo de projeto** ([seção 11](#11-memos)). Ele orienta a codificação. Para orientar a IA, escreva o **Memo para a IA** (ou marque o memo do projeto no ⚙ Configurar Prompt).
...
- **Membro**: codifica na sua camada, preenche as próprias respostas de categoria, **adiciona documentos**, **cria e renomeia** códigos, edita o **verbete** deles, escreve memos e usa os painéis de IA (quando o projeto tem IA).
...
> Esses são os valores **padrão**. Num projeto coletivo, quem cria decide item a item o que membro e somente leitura podem fazer, e o desenho do estudo (como a codificação cega): veja [O que cada papel pode fazer](#o-que-cada-papel-pode-fazer-decidido-na-criação).

> **Como atribuir:** em **Projeto ▸ Membros & administradores**, o admin clica em **tornar somente leitura** na linha da pessoa (e em **devolver a codificação** para desfazer). Ela entra no projeto com o mesmo código de acesso de sempre. O projeto nunca fica sem admin: o botão recusa rebaixar o último que existe. E a **configuração do estudo** — as instruções da IA, a postura, os prompts salvos e as palavras ignoradas na nuvem — é guardada como memo mas **não** é comentário: fica com quem coordena.
...
Ao baixar o arquivo único, ele abre direto no navegador (`file://`) sem precisar de servidor, e abre sem internet. Só quando você usa PDF, DOCX, planilha, OCR ou busca semântica ele baixa a biblioteca correspondente (o navegador costuma guardá-la em cache). (Se a sua política de navegador bloquear, sirva com `python -m http.server 8000` na pasta do arquivo.)

> **Chrome ou Edge** são recomendados: só neles funciona o **modo Arquivo local** (salvar um `.qualilab` visível no disco) e o **backup automático em pasta**. Firefox e Safari funcionam em **nuvem** ou **rascunho** (`localStorage`); só o modo arquivo fica indisponível neles.
...
Ao criar um projeto (rascunho, arquivo ou nuvem), o QualiLab pergunta se ele terá **recursos de IA**: **Sem IA** (o padrão: as telas de IA não aparecem, e a Auto-codificação fica só com a Repetir Codificação, que não usa IA) ou **Ativar IA**. Dá para mudar depois ([17.7](#177-desligar-a-ia-neste-projeto)).

Quando o navegador permite instalar, a tela de entrada mostra também **Instalar o QualiLab** (no Windows com Chrome, em destaque **Instalar pelo Edge**): veja [Instalar como aplicativo](#instalar-como-aplicativo-e-abrir-sem-internet).

A interface abre em português se o navegador estiver em português, e em inglês nos demais casos; troque em **Minha conta ▸ Idioma** ([seção 14](#idioma-da-interface)).
...
> Um projeto em nuvem ou arquivo já aberto **reabre na próxima vez**; se o navegador pedir de novo a permissão do arquivo, a tela de entrada mostra **Reabrir *nome*.qualilab**, e um clique basta. Se o app não tiver nuvem configurada, "Entrar na nuvem" não aparece e você começa direto em arquivo/rascunho.

### 2.3. Escolher / criar um projeto

Depois do login aparece **"Meus projetos"**:
...
- **Criar projeto**:
  ...
  4. Decida os **Recursos de IA**: o projeto nasce **Sem IA** (veja [17.7](#177-desligar-a-ia-neste-projeto)).
  5. *(Coletivo)* Escolha o **tipo de estudo** (equipe aberta, confiabilidade entre codificadores, dividir o corpus, painel de juízes) e ajuste a matriz **O que cada papel pode fazer**. Decida antes de convidar ([seção 13](#o-que-cada-papel-pode-fazer-decidido-na-criação)).
  6. Clique em **Criar**. No coletivo, o hub do projeto abre em seguida, já nos convites.
- **Entrar com código**: para participar de um projeto coletivo de outra pessoa, cole o **código de acesso** ou de **convite** (10 caracteres, ex.: `9F2A1C7B40`) e clique em **Entrar**.
...
| **Reconciliação** | *(só projeto coletivo; some sob codificação cega e no modo foco)* consolidar o gabarito |
| **Leitura** | Ler o documento inteiro, ou todos os trechos de um código |
| **Memos** | Notas analíticas |
| **Esquema** | Organizar códigos e categorias em lote *(some no modo foco)* |
...
> As três telas de **IA** são **opt-in**: com a IA desativada no projeto (o padrão), **Analisar com IA** e **Explorar com IA** não aparecem, e a **Auto-codificação** fica só com a Repetir Codificação, que não usa IA. Nenhuma das três aparece para quem tem papel de somente leitura. Detalhes na [seção 17](#17-codificar-e-analisar-com-ia).

**Segunda linha**
- A **pílula do projeto**, ex.: `rascunho · Meu Projeto · individual ▾`. O prefixo mostra o modo de armazenamento (`arquivo`/`nuvem`/`nuvem pessoal`/`rascunho`) e a **cor** reforça onde os dados estão: neutra = rascunho (neste navegador), verde = arquivo no seu disco, azul = nuvem (servidor padrão), violeta = nuvem no seu próprio Supabase, âmbar = nuvem sem conexão. Passe o mouse para a explicação (em modo rascunho, inclui o % usado do armazenamento do navegador); clicar abre o **hub de gestão do projeto**. Na nuvem, a pílula traz também o código de acesso.
- O **selo de IA** do projeto, logo depois da pílula: anel **verde** quando os recursos de IA estão ativados; **barrado em vermelho** quando estão desativados ou restritos aos administradores (passe o mouse para ver qual). Só administradores trocam, com confirmação; para os demais, o clique explica o estado (veja [17.7](#177-desligar-a-ia-neste-projeto)).
...
- *(Admin, projeto coletivo)* **admin / ● foco**: o modo foco esconde a Reconciliação, o Esquema e o filtro **Ver:**, para você codificar como qualquer membro (suas permissões não mudam, só a sua tela).
- *(Membro, com distribuição de documentos)* a pílula **N documento(s) seu(s)**.
- **trocar projeto** / **sair** (modo nuvem).
- **exportar ▾** (quando há documentos) e **importar ▾** (sempre, exceto para quem tem papel de somente leitura).
...
- **✓ salvo HH:MM** (arquivo/rascunho) ou o estado da nuvem: `✓ nuvem em dia`, `offline`, e —
  ...
  (veja a [seção 16](#16-salvamento-backup-e-modos-de-armazenamento)). Se a gravação local
  falhar, aparece **✕ não salvo · baixar backup**, também clicável.
...
Logo abaixo do cabeçalho podem aparecer **faixas de aviso**: no rascunho, a âmbar lembrando que os dados ficam só neste navegador (com **Salvar como arquivo** e **Conectar à nuvem**); erro (vermelho); resumo de importação; importação em andamento (com barra de progresso); falha de salvamento; e alterações que a nuvem recusou (veja a [seção 16](#16-salvamento-backup-e-modos-de-armazenamento)).
...
| Configuração de IA (à esquerda) | as abas da Auto-codificação e a Analisar com IA |
...
- **Janelas de diálogo**: ao abrir, o foco entra na janela; **Tab** e **Shift+Tab** circulam **dentro
  ...
  abriu.
- **Telas estreitas** (celular, abaixo de ~720 px): aparece um aviso de tela pequena. Dá para **ler
  e consultar**, mas não trabalhar com conforto: o leitor e os painéis não cabem lado a lado. Em
  tablet, selecionar um trecho por toque abre a barra flutuante de códigos. Para trabalhar de verdade,
  use um computador.
...
### Enviar arquivos
Na aba **Codificação**, no topo do leitor, clique em **＋ enviar ▾ → enviar arquivo(s)…** e escolha um ou mais arquivos `.txt`, `.md`, `.docx` ou `.pdf`. O texto é extraído e exibido para leitura.
...
### Colar texto
Em **＋ enviar ▾ → colar texto…** você cria um documento a partir de texto copiado, sem arquivo (num projeto vazio, o botão **colar texto…** aparece no meio do leitor).
...
- **🗑 excluir documento** remove o documento aberto e todas as suas codificações. Não há Ctrl+Z; mas, com o espelho automático ligado, o QualiLab guarda o projeto antes da exclusão (veja [Espelhos do projeto](#espelhos-do-projeto-ponto-de-restauração)). Em projeto coletivo na nuvem, excluir documento é do administrador.
...
Esta é a tela **Codificação**: leitor à esquerda e, à direita, um painel com três abas: **⍞ Categorias**, **✎ Codificar** (a árvore de códigos) e **▤ Memos**.

### 5.1. Criar códigos
Na aba **✎ Codificar** (direita) você cria e organiza os rótulos. Um código novo nasce solto em **Hierarquia 0** e já recebe trechos. Você pode criar subcódigos, renomear e excluir. Também dá para criar um código **na hora de aplicar** (veja abaixo).
...
codificação**: com um trecho selecionado, o clique esquerdo *aplica* o código, então esse menu (ou
os botões ✎ e ＋ ao lado de cada código no menu de aplicar) é o caminho para o editor sem desfazer a
seleção.

**O verbete do código.** Ao selecionar um código na aba ✎ Codificar, o memo dele (a definição: o que
conta e o que não conta) aparece logo abaixo, só para leitura; para editar, use a aba ▤ Memos. Com um
trecho já selecionado, clicar no código o aplica, então consulte o verbete antes de selecionar.
...
3. No menu, clique no código desejado; ele é aplicado na hora. O campo **buscar código...** filtra
   pelo caminho inteiro; **↑ ↓** movem e **Enter** aplica. Com o trecho selecionado, clicar no código
   na aba **✎ Codificar** também aplica.
   - Ou clique em **+ Criar novo código**: digite o nome, escolha **(no topo)** ou **dentro de "…"** e clique em **Criar e aplicar**. Criar dentro de um código que já tem trechos o transforma em família (veja 5.1).
   - Com um trecho selecionado, as teclas **1 a 5** aplicam um dos até cinco códigos **recentes**
     da sessão (a lista numerada fica na aba ✎ Codificar, à direita).
...
No mesmo menu, **⇄ Alterar código** troca o código do grifo sem refazer a seleção, e **Anotar trecho
(nota analítica)** abre a nota daquele trecho. Atalho: clique no grifo e aperte **Delete**; aparece um
aviso com **desfazer**.

> Em projeto coletivo na nuvem, você só remove codificações **suas** (o administrador pode rejeitar a de qualquer um). Não dá para apagar o grifo de outro pesquisador. (Em projeto individual, tudo é seu.)

### 5.4. Desfazer (Ctrl+Z)
**Ctrl+Z** desfaz a **última codificação aplicada**, ou a última removida com **Delete**, na sessão atual (até as últimas 50). Funciona só na aba Codificação e fora de campos de texto. Excluir documento, categoria ou código, mesclar e dividir não têm Ctrl+Z; com o espelho automático ligado, o QualiLab guarda o projeto antes delas e dá para restaurar (veja [Espelhos do projeto](#espelhos-do-projeto-ponto-de-restauração)).
...
- **⬌ / ⬍**: alterna entre a coluna estreita de leitura (~65 caracteres, o padrão) e a coluna cheia.
...
No **pesquisar +** há um quarto botão, **≈ termos**, que resolve o problema de *não saber com que palavra o material fala do assunto*. Você digita a ideia que procura (uma ou duas palavras bastam) e ele mostra **as palavras do seu próprio corpus** que estão no mesmo campo de sentido, com quantas vezes cada uma aparece.
...
- **Individuais (todos)**: sobrepõe os grifos de todos os pesquisadores e os do gabarito. Você continua trabalhando normalmente aqui: os grifos que você aplicar são seus, e o campo de cada categoria mostra e edita a **sua** resposta, com as respostas dos colegas e o gabarito logo abaixo, apenas para leitura.
- **Minhas**: só o seu trabalho (nas categorias, o valor do gabarito, se houver, aparece abaixo do campo, só para leitura).
...
Em projeto individual com mais de um autor importado, o seletor mostra **Todos os codificadores** e o nome de cada autor. Sob **codificação cega** o seletor não aparece (no lugar dele fica a pílula **codificação cega**).
...
Cada categoria pode ter uma **descrição/instrução** e habilitar opções especiais: **"Não informado"** (todos os tipos) e, nos tipos com lista de valores, **"Outros"** (com valor livre).

Clique no nome de uma categoria para **recolhê-la**: a linha continua mostrando o valor (ou "—"). Se você ainda não respondeu e existe gabarito, a linha recolhida mostra o valor do gabarito **em itálico**; o campo continua vazio de propósito, porque ele é a **sua** resposta.
...
- **Preencher**: quem pode codificar (pela [matriz de regras](#o-que-cada-papel-pode-fazer-decidido-na-criação) do projeto) responde a **sua** versão; o somente leitura não preenche; o admin define o **gabarito**.
...
- **Clique simples em um código** (na linha, não na caixa) → editar o nome (e, se for admin, a cor e a censura da família de topo) + **Promover a Hierarquia 0** (se for subcódigo) + a seção **Conexões** + **⑃ Dividir em subcódigos** (se ele tiver trechos).
- **Marque 2 ou mais** (caixas) → aparecem duas ações:
  ...
  - **Mesclar**: escolhe um **sobrevivente** (sugestão = o mais frequente); as codificações dos demais são **reatribuídas** a ele e os outros são excluídos. **Só administradores** (reatribui codificações de todos). Não há Ctrl+Z; o app confirma antes e, com o espelho automático ligado, espelha o projeto antes. Os filhos dos mesclados são preservados (passam para o sobrevivente).
...
**Onde fica.** Com um código selecionado, a seção **Conexões** aparece no painel da direita, abaixo do editor do código. Para uma categoria, ela está dentro do card dela, em **Esquema ▸ Categorias**, recolhida num "Conexões" que você abre (só o administrador, que é quem edita o esquema de categorias, vê as conexões de categoria ali).
...
*Só em projeto coletivo, e não aparece para quem está sob codificação cega. Se o seu projeto é individual, pule para a [seção 9](#9-leitura).* É onde a equipe consolida o **gabarito** a partir do trabalho individual de cada pesquisador. A coluna da esquerda tem sempre as duas seções, **Categorias** e **Códigos**, cada uma com duas listas que abrem e fecham: **Documentos** (reconciliar tudo de um documento, ou **(Todos os documentos)** para o projeto inteiro de uma vez) e **Categorias** ou **Códigos** (reconciliar uma categoria, ou um código, em todos os documentos). As listas começam recolhidas, e a recolhida mostra ao lado do título o que está escolhido nela. Clicar num item já leva à seção dele.

**Categorias.** Para cada documento e categoria, você vê o **Gabarito** (que o admin define) e, abaixo, a resposta de cada pesquisador com **✓** (igual ao gabarito), **✗** (diferente) ou **·** (ainda não há gabarito). Sem gabarito, cada categoria diz se os codificadores **concordam** ou **divergem** entre si (resposta em branco não conta). Em **Sua resposta**, qualquer codificador registra ou muda a sua, sem mexer no gabarito. No modo (Todos os documentos), escolhe-se uma categoria e ela é consolidada documento a documento.
...
- **Discordo**: registra que o código **não** se aplica ao trecho, sem apagar a codificação de ninguém. Está aqui ao lado do **Concordo** e no botão direito da Leitura, e as duas telas gravam o mesmo veredito. Dado o veredito, o card diz **você discorda**, com **desfazer**. Cada grupo mostra também **quem discorda** ("2 discordam:" e os nomes), e o selo de **consenso** não acende enquanto houver alguém discordando: "2 de 3" deixa de esconder se o terceiro recusou o código ou só não olhou.
...
- **Passar o mouse** num grifo mostra o código, quem o aplicou e o começo da nota analítica, quando
  houver; **clicar** abre uma faixa embaixo
...
- Ao abrir, a Leitura mostra o documento que está aberto na Codificação; escolher outro na lista não
  muda a Codificação. O botão **codificar →** abre na Codificação o documento que você está lendo.
...
cega, nem para quem não pode codificar neste projeto (o papel de somente leitura, ou um membro sem
"codificar" na [matriz de papéis](#o-que-cada-papel-pode-fazer-decidido-na-criação)).
...
- **Nota analítica (●)**: um balão com **●** avisa que aquele trecho tem uma nota analítica; clique no **●** para **ler a nota ali mesmo**. (A nota se escreve pelo botão direito no leitor da **Codificação**, em "Anotar trecho (nota analítica)", ou na aba Memos.)
- **Abrir no documento**: clique no **texto do trecho** (ou no nome do documento) para abri-lo na sub-aba **Documentos**, aqui mesmo, rolando até o grifo, que pisca para você achar. Para ir à Codificação, clique no grifo e use **abrir na Codificação →**. Com a caixa **grifos** desmarcada, o documento abre no topo.
- **Filtro por categoria**: restringe aos documentos que atendem certos atributos.
- **Co-ocorrência com:** mostra só os trechos em que o código selecionado se sobrepõe a outro (ou a mais dois, com **e também…**).
...
No alto, ao lado das abas, ficam **Ver:** e **Top:** (10/25/50/Todos; na Nuvem, **Palavras:**), que refinam o recorte.
...
**Seções de IA** (abaixo das anteriores). **Memo para a IA** e **Prompts salvos** somem quando a IA está desativada no projeto; **Conversas salvas** e **Memória do projeto** ficam sempre, porque são registro do que houve.
...
A última entrada da coluna, fora das seções de IA, é o **Histórico do projeto**: a **trilha de auditoria do processo**. É uma lista de frases, da mais recente para a mais antiga, com o que **alterou** o projeto, cada uma com data e autor: importações e mesclagens de arquivo, mesclagens e divisões de código, exclusões (documento, código, categoria, limpar conteúdo), edições de texto com reancoragem dos grifos, aplicações em lote (Repetir Codificação e as telas de IA), consolidações da Reconciliação, mudanças de tipo e configuração, criação e restauração de espelhos, renomeação do projeto, entradas, saídas e mudanças de papel de membros (na nuvem) e exportações. Há um **filtro por operação** e uma **busca por texto**: digite o nome de um código para ver o que já aconteceu com ele. **Exportar CSV** baixa a lista inteira (frase legível + detalhe em JSON).
...
A aba **Relatório** é o **hub de publicação**. Na coluna esquerda você escolhe entre três saídas. Em projeto coletivo, o Relatório Padrão segue o recorte **Ver** escolhido no painel; o Interativo (ATI) e o W3C usam o gabarito quando ele existe e, sem gabarito, as camadas individuais. Em todas, trechos de **censura** saem mascarados. As saídas de transparência (ATI e W3C) ainda oferecem **Ocultar autoria (revisão cega)**, que omite os nomes de quem codificou, útil para publicar sem expor a equipe. *Atenção: isso não anonimiza o conteúdo dos documentos. Veja [Dados sensíveis e responsabilidade](#dados-sensíveis-e-responsabilidade).*
...
Para criar a cópia: **exportar ▾ → .qualilab**, depois crie um projeto novo (na nuvem: **trocar projeto → Criar projeto**; no modo arquivo: pílula do projeto → **Sair deste arquivo** → **Novo em arquivo**) e **importar ▾ → .qualilab** nele. Agora limpe a cópia:
...
No painel esquerdo há a caixa **"Incluir informações sobre uso de IA"**, **ligada por padrão** (vale nas três saídas). Com ela marcada, as três saídas — Relatório Padrão, Relatório Interativo e Web Annotation — levam um bloco curto com **o que o seu projeto registra**.
...
### Convidar pessoas
Abra a **pílula do projeto** (cabeçalho) → ali está o **código de acesso**. Compartilhe-o; quem recebe entra por **"Meus projetos" → Entrar com código**. No modo nuvem, o código também aparece na própria pílula (`nuvem · Projeto · CÓDIGO · coletivo ▾`). Quem entra pelo código de acesso entra sempre como **membro**.
...
- Ao escolher o papel, o card mostra o que quem entrar com ele vai poder fazer neste projeto: a mesma lista que vai no e-mail, gerada da [matriz de papéis](#o-que-cada-papel-pode-fazer-decidido-na-criação).
- **O QualiLab não envia e-mail nenhum.** A entrega é sua: copie o código e mande pelo canal que quiser, ou, se o convite tiver e-mail, clique em **abrir e-mail** — isso só monta a mensagem (com o código e o papel já escritos) e abre **a sua própria** caixa de saída, no seu programa de e-mail. Nada passa pelo servidor.
...
Ainda na pílula do projeto, o admin pode: ver a **lista de membros** e mudar papéis (**administrador**, **membro** ou **somente leitura**; o projeto nunca fica sem administrador), **renomear**, **limpar conteúdo**, **excluir** o projeto, mudar o **tipo** e ajustar a **conexão** (credenciais Supabase).
...
Por que na criação: o que alguém já viu não se desvê. Um projeto que nasce aberto e só depois liga a codificação cega já contaminou quem entrou antes. Com a matriz decidida no início, o **convite** já vai com as regras em vigor — e o e-mail do convite lista o que aquele papel vai poder fazer, gerado da própria matriz. É o **servidor** que aplica cada linha (não a tela): um membro sem "codificar" recebe recusa do banco, não só um botão escondido. Dá para mudar depois, no hub do projeto (**Distribuição e sigilo → Distribuir documentos… → Como este projeto controla o acesso**); a mudança entra no histórico do projeto. Sem mexer em nada, o comportamento é o de sempre: membro faz tudo, somente leitura lê e comenta.

Desmarcar **Edita o verbete dos códigos** para os dois papéis **trava o verbete**: só a administração edita a definição dos códigos (ler continua livre), o hub mostra a pílula **verbete travado** e ligar ou desligar fica no histórico. A linha **Usa os painéis de IA** fica desabilitada quando a IA do projeto está desativada ou restrita à administração.

### Distribuir documentos e codificação cega
*(Só admin, projeto coletivo na nuvem.)* No hub do projeto, o card **Distribuição e sigilo → Distribuir documentos…** abre uma **matriz de documentos × pesquisadores**, onde você marca quem codifica o quê. Um selo **C** mostra quem **já codificou** cada documento; o botão de **rodízio automático** distribui tudo de uma vez (1 ou mais pessoas por documento). Sozinha, a matriz é só um **plano de trabalho**: ela vira regra quando você marca, na parte **Desenho do estudo** da matriz de acesso no alto do mesmo modal, uma das duas decisões (**independentes**):
...
### Enviar para a nuvem
Se o projeto ativo for **rascunho** ou **arquivo**, a pílula mostra **"Enviar para a nuvem"**: cria um projeto novo na nuvem e copia tudo (documentos, categorias, códigos, codificações, memos) de uma vez, sem exportar/importar `.qualilab` na mão. O formulário pergunta o tipo do projeto e, se for coletivo, mostra a mesma matriz de papéis. Os **PDFs originais** só sobem se você marcar **Enviar também os PDFs originais**.
...
- Escolher o **idioma da interface**: automático (do navegador), português ou inglês (ver abaixo).
...
nada: o seletor em **Minha conta** serve para escolher o contrário, ou voltar ao automático. Trocar **recarrega a**
**página**.
...
Os menus ficam no cabeçalho. **exportar ▾** aparece quando há documentos; **importar ▾** aparece sempre (menos para o papel somente leitura), e os dois itens de planilha que trabalham sobre documentos existentes só aparecem quando já há documentos.

### Exportar (menu "exportar ▾")
| Item | O que é |
|---|---|
| **.qualilab (projeto completo, nativo)** | Tudo (documentos e PDFs originais, categorias, valores, códigos, codificações, memos, conexões, discordâncias, conversas e memórias de IA e o histórico do projeto) para reabrir no QualiLab. É o backup completo do projeto |
| **JSON (projeto)** | Documentos, categorias e valores, códigos e codificações (com camada, autor e página), num JSON legível por script. Não leva memos, IA, histórico nem conexões: para backup, use o `.qualilab` |
...
> **Antes de gerar o QDPX**, o QualiLab lista o que não cabe no formato, cada item só quando o projeto o tem (codificações individuais quando há gabarito, respostas de mais de uma pessoa, caixas de seleção, o histórico, os trechos de evidência das conexões e as discordâncias), e pede confirmação. As **conexões** entre códigos e categorias viajam como `<Link>`.
...
- O texto sai do PDF do mesmo jeito que no `＋ enviar`, então tudo o que depende do PDF funciona igual. **PDF sem camada de texto (escaneado) entra vazio de propósito**: abra o documento e use **⋯ → OCR: ler páginas escaneadas** (disponível se você guardou o PDF original).
...
1. **importar ▾ → planilha (.csv / .xlsx → documentos + categorias)** e escolha o arquivo.
2. No **modal de mapeamento**, para cada coluna escolha o papel: *(ignorar)*, **Texto (conteúdo do documento)**, **Nome do documento**, **Memo do documento**, ou **Categoria · <tipo>**.
...
também em ⋯ → **Aplicativos** → **Instalar este site como um aplicativo**). O motivo do Edge
é concreto: ele registra o QualiLab no sistema como um aplicativo completo, e o arquivo
`.qualilab` ganha **nome de tipo e ícone próprios** no Explorer. Instalando pelo Chrome tudo
funciona, mas o `.qualilab` fica com o **ícone genérico de folha em branco** (uma limitação do
Chrome no Windows, não do QualiLab). No macOS esse problema do ícone não deve ocorrer.
...
O projeto é um arquivo `.qualilab` **visível no sistema de arquivos** (qualquer pasta, HD externo, volume criptografado). Zero rede, zero `localStorage`, 100% offline. Comece na tela de entrada, em **Novo em arquivo** ou **Abrir arquivo existente** (ou, logado na nuvem, em **Meus projetos → Novo arquivo… / Abrir arquivo…**). O app reabre o último arquivo na sessão seguinte (com permissão do navegador).
...
- **Seu próprio Supabase** (a pílula do projeto fica **violeta**, "nuvem pessoal"): você aponta o app para um projeto Supabase **seu** (criar um é gratuito). O banco passa a ser seu; só você e quem você autorizar têm as chaves. Continua hospedado pela empresa Supabase, mas o dono dos dados é você. Configure na tela de entrada ou de login, em **Conectar ao meu Supabase**, ou, fora do modo nuvem, no hub do projeto, em **Conexão (Supabase)**.

**O que protege os seus dados na nuvem:**
- **Login** (e-mail e senha, ou conta Google, via Supabase Auth): só quem tem conta entra.
...
- **Excluir o projeto é definitivo** (baixe um `.qualilab` antes). Antes de excluir documento, código ou categoria, ou de limpar o conteúdo, o QualiLab guarda um [espelho](#espelhos-do-projeto-ponto-de-restauração) automático, se o interruptor estiver ligado.
...
Um **espelho** é o projeto inteiro num instante (documentos, categorias, códigos, codificações, memos, conexões, discordâncias, conversas e memórias de IA), guardado ao lado do projeto para **restaurar** depois. Fica em **pílula do projeto → Espelhos do projeto**.

- **Espelhar agora** (com um rótulo opcional) cria um espelho manual; manuais ficam até você excluir.
- **Automáticos**: antes de limpar o conteúdo, excluir documento, código ou categoria, mesclar ou dividir código (no Esquema ou pela IA) e editar o texto de um documento, o QualiLab espelha sozinho, no máximo um a cada 10 minutos. Antes de **restaurar**, espelha sempre, mesmo com o automático desligado. Ficam os 5 automáticos mais recentes.
...
### Modo nuvem offline
A pílula do projeto fica âmbar quando a conexão cai, e a **barra de status, no rodapé**, mostra `offline` ou, havendo pendências, **quantas alterações estão aguardando envio** (clique para tentar na hora). **Você pode continuar codificando**: o que você escreve fica guardado neste navegador e sobe sozinho quando a nuvem voltar a responder (é a fila descrita no quadro acima, e ela sobrevive a fechar a aba).
...
- **Ver e configurar o prompt (⚙).** As abas da *Auto-codificação* (menos a Repetir Codificação, que não usa IA) e o *Analisar com IA* têm, no topo, o botão **⚙ Configurar Prompt**. Ele abre a **prévia exata** do que será enviado, seção por seção (papel, memo, memória, material, regras, tarefa), com o modelo ativo, as contagens, a **estimativa de tokens e de custo (≈ R$)** e um botão **copiar prompt**. No mesmo painel você ajusta o que a IA recebe: nas telas de *Auto-codificação*, as **instruções próprias à IA**, os **memos injetados** e a **memória do projeto**; em *Analisar*, isso **e** a **postura** metodológica (veja [17.3](#173-analisar-com-ia-leitura-assistida-do-material)). **Nada sai dessas telas sem passar por aqui**. É a face concreta da regra de transparência. Na *Explorar com IA* não há prévia: o que sai são as chamadas de leitura, que a tela lista uma a uma (veja [17.6](#176-explorar-com-ia-a-ia-pede-o-material-em-vez-de-receber)).
...
- **Censura sempre antes do envio.** Trechos de códigos marcados como **censura** ([5.5](#55-censura-mascarar-trechos-sensíveis)) são substituídos por `[trecho censurado]` (na *Explorar com IA*, por blocos `█` do mesmo comprimento, para as posições continuarem válidas) **antes** de o material sair do navegador. Em *Analisar com IA*, você pode optar por incluir um código de censura específico naquela análise (opt-in explícito, por código).
...
![Configurar Prompt, aberto na Auto-codificação: em cima, os controles (instruções próprias à IA, memos injetados e a memória do projeto, agora dentro do modal); embaixo, a prévia do que será enviado (modelo, material em páginas, censura mascarada e estimativa de tokens e de custo (≈ R$)) e o prompt seção por seção, com "Papel e princípios" mostrando as regras invioláveis da IA (a IA propõe, não inventa, cita a fonte).](manual-img/15-ia-configurar-prompt.png)
...
- **Documentos** (texto integral dos documentos escolhidos): *Temas candidatos* · *Camada latente* · *Síntese analítica* · *O inesperado* · *Diferenças entre casos* · *Variação por categoria*.
- **Trechos + Código** (os trechos de cada código, tratado como categoria analítica): *O que há no código* · *Coerência e saturação* · *Código vs. definição* · *Diferenças entre casos* · *Memo analítico*.
- **Documentos + Trechos + Código** (a codificação lida em contexto, uma "segunda leitura"): *Quais códigos complementariam a análise* · *Quais códigos desafiam a análise* · *Validação em contexto* · *Trecho em contexto* · *Síntese contextualizada* · *Variação por categoria*.
...
- **Instruções próprias à IA**: guias que entram em **todo** prompt (ex.: "priorize a linguagem dos entrevistados"). *(Compartilhadas com a Auto-codificação.)*
...
O mesmo card traz ainda, recolhidos em **Custo estimado (calculadora)**, o **câmbio US$→R$** e, para provedores sem tabela de preços (*Personalizado*/*Azure*), a **sua tarifa** por milhão de tokens, usados na estimativa de custo ([17.1](#171-como-a-ia-funciona-aqui)).
...
**A pergunta vem na criação.** Todo projeto novo — na nuvem, em arquivo ou rascunho — pergunta se os recursos de IA ficam disponíveis. A opção que **já vem marcada é "Sem IA"**: ativar exige trocar a escolha, e sair da janela sem responder **mantém desligado**. No cabeçalho, **entre a pílula do projeto e o seu nome**, um selo **IA** mostra sempre o estado da IA neste projeto: anel verde quando ativada; anel vermelho com barra quando desativada ou restrita aos administradores. Administradores clicam nele para trocar, com confirmação. Você também muda em **Projeto → Recursos de IA** (clique na pílula do projeto):
...
Além desta chave, a [matriz de papéis](#o-que-cada-papel-pode-fazer-decidido-na-criação) (*Usa os painéis de IA*) pode tirar as telas de IA de membros mesmo com a IA ativada, com o mesmo recorte (a Auto-codificação fica, só com a Repetir Codificação). Para quem tem papel **somente leitura** as três nunca aparecem, a Repetir inclusive, porque esse papel não codifica.

**O que some, e o que fica.** Somem do cabeçalho **Analisar com IA** e **Explorar com IA**, e some a configuração que só serve às telas de IA (o **Memo para a IA** e os **Prompts salvos**, na tela Memos). A **Auto-codificação** fica, só com a aba [Repetir Codificação](#1725-repetir-codificação-sem-ia), que não usa IA nenhuma e é a ferramenta que a [seção 5.5](#55-censura-mascarar-trechos-sensíveis) recomenda para fechar a censura (achar as outras ocorrências idênticas de um termo já censurado); uma linha no topo da aba explica por que as outras não aparecem. (Até a v1.4.64 a tela saía inteira, e a Repetir junto.) **Fica** também o registro do que já aconteceu: as **conversas salvas** e a **memória do projeto** continuam onde estavam. Esconder o registro seria o oposto da transparência que a declaração promete — e é ele que o relatório conta.
...
O app abre sem internet; só os recursos pesados (ler PDF, DOCX ou planilha, OCR, busca semântica) e a nuvem precisam de rede no primeiro uso. Tela em branco costuma ser o navegador bloqueando o `file://`: sirva por um servidor local, com `python -m http.server 8000` na pasta do `index.html`.

**Com o app instalado, trocar de tela abre uma janela do navegador.**
...
O armazenamento encheu ou ficou indisponível. Clique em **baixar .qualilab** imediatamente; depois libere espaço (o rascunho tem limite de ~5 MB) ou migre para o modo **arquivo**/**nuvem**.
...
Abra **pílula do projeto → Espelhos do projeto**: se o espelho automático estava ligado, há um retrato de antes da exclusão, e **Restaurar** devolve o projeto àquele instante (os PDFs originais não voltam). Sem espelho, recupere de um `.qualilab` exportado. O Ctrl+Z só desfaz a última *codificação de trecho*.
...
| **1 … 5** | Codificação, com trecho selecionado | Aplicar um dos 5 códigos recentes da sessão (a lista numerada no painel) |
| **Ctrl+Z** | Codificação | Desfazer a última codificação (aplicada ou apagada) |
| **Enter** / **Shift+Enter** | Busca (**pesquisar**) | Próxima / anterior ocorrência |
...
| **Esc** | Em qualquer tela | Fechar o que estiver por cima: menu, busca, paleta, filtro, diálogo (num diálogo, o foco volta ao botão que o abriu) |
| **← →** | Barra de telas | Ir à tela anterior / seguinte |
...
- **Conexão**: relação nomeada entre dois códigos, ou entre código e categoria, como *contradiz* ou *é parte de*; é anotação, não medida, ver [7.5](#75-conexões-dizer-o-que-um-código-tem-a-ver-com-outro).
- **Discordância**: o veredito *Discordo* sobre o código de um colega, na Leitura ou na Reconciliação; não apaga nada nem entra nas contagens, ver [9](#9-leitura).
- **Espelho**: retrato do projeto inteiro num instante, para restaurar depois, ver [16](#espelhos-do-projeto-ponto-de-restauração).
- **Modo (armazenamento)**: onde os dados ficam (arquivo, rascunho ou nuvem).
- **Tipo de projeto**: individual (sem reconciliação) ou coletivo.
- **Papel**: administrador (esquema, gabarito, membros, espelhos), membro (codifica) ou somente leitura (lê e comenta em memos). O que cada papel pode fazer se ajusta item a item na matriz do projeto, ver [13](#o-que-cada-papel-pode-fazer-decidido-na-criação).
