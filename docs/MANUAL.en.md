...
When you create a project (draft, file or cloud), QualiLab asks whether it will have **AI features** ("recursos de IA"): **No AI** ("Sem IA"; the default: the AI screens do not appear, and Auto-coding keeps only Repeat Coding, which uses no AI) or **Turn AI on** ("Ativar IA"). You can change it later ([17.7](#177-turning-ai-off-in-this-project)).
...
> The three **AI** screens are **opt-in**: with AI turned off in the project (the default), **Analyze with AI** and **Explore with AI** do not appear, and **Auto-coding** keeps only Repeat Coding, which uses no AI. None of the three appears for anyone with the read-only role. Details in [section 17](#17-coding-and-analyzing-with-ai).
...
- **I disagree** ("Discordo"): records that the code does **not** apply to the passage, without deleting anyone's coding. It is here next to **I agree** and on the right-click in Reading, and both screens record the same verdict. Once given, the card says **you disagree**, with **undo**. Each group also shows **who disagrees** ("2 disagree:" and the names), and the **consensus** badge does not light up while someone disagrees: "2 of 3" no longer hides whether the third person rejected the code or simply did not look.
...
Beyond this switch, the [role matrix](#what-each-role-can-do-decided-at-creation) (*Uses the AI panels*) can take the AI screens away from members even with AI on, with the same cut (Auto-coding stays, with only Repeat Coding). For someone with the **read-only** role none of the three appears, Repeat Coding included, because that role does not code.

**What disappears, and what stays.** **Analyze with AI** and **Explore with AI** leave the header, along with the configuration that only serves the AI screens (the **Memo for the AI** and the **Saved prompts**, on the Memos screen). **Auto-coding** stays, with only the [Repeat Coding](#1725-repeat-coding-no-ai) tab, which uses no AI at all and is the tool [section 5.5](#55-redaction-masking-sensitive-passages) recommends for closing the redaction (finding the other identical occurrences of an already-redacted term); a line at the top of the tab explains why the others do not appear. (Until v1.4.64 the screen left whole, and Repeat Coding with it.) What also **stays** is the record of what already happened: the **saved conversations** and the **project memory** remain where they were. Hiding the record would be the opposite of the transparency the statement promises — and it is what the report counts.
...
- **Disagreement**: the *I disagree* verdict on a colleague's code, in Reading or Reconciliation; it deletes nothing and does not enter counts, see [9](#9-reading).
